...
- **Dataset:** Generated like iteration 2 (self-play in Nuxt, `playGame()` in `checkers-web/src/server/utils/scrape.ts`), with parameters e.g. **20,000 games**, **modelLevel 2** (engine_2), **random 0.12**, **depth 4**. **Random move probability** is variable: early in the game (first few turns) the probability of a random move is higher and decreases linearly to the `random` value; from a certain turn (e.g. 5) a constant `randomCoefficient` is used. **Minimax depth** (`depth`) is configurable (0 = evaluation only, no minimax); when `depth > 0`, move choice uses `pickBestEngineContinuation(..., depth)`. Position and evaluation recording as in iteration 2 (including turn filtering via `shouldSaveMove`). Result distribution (−1 / 0 / 1) can be computed from the generated JSON with `ml-core/dataset/result_spread.py`.
